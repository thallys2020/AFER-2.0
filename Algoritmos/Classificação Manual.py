...
f = open('ExpectedResultsF.csv', 'w', newline='', encoding='utf-8')

with open('ExpectedResultsF.csv', 'w', newline='') as file:
            ...
            if s > 1:
                ...
                if (not line[10] == "-"):
                    ...
                    if not(EmotionFeminino(result) == None):
                        print(line[0], line[2], line[12], EmotionFeminino(result))
                        
                        l = open('ExpectedResultsF.csv', 'a', newline = '')
                        try:
                            writer = csv.writer(l, delimiter = ";")
                            writer.writerow((line[0], line[2], line[12], EmotionFeminino(result)))
                        finally:
                            l.close()
                        
                        if line[12] == str(EmotionFeminino(result)):
                            i = i + 1
                        j = j + 1  
